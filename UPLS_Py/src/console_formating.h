...
namespace na::Console
{
	...
	public:
	    ...
	    friend std::ostream& operator << (std::basic_ostream<_CharT>& os, const Format& obj)
	    {
			bool isTerminal = Format::m_isTerminal(&os);
			
			if (obj.m_reset && isTerminal)
	        {
	            os << "\033[0m";
	        }
	        else
	        {
	            if (isTerminal)
				{
	            	...
	            	{
	                	if (!semicolonNeeded) semicolonNeeded = true;
	                	else os << ';';
	...
	private:
		...
		template<typename _CharT>
		static bool m_isTerminal(std::basic_ostream<_CharT>* os)
		{
			using namespace __gnu_cxx;
			stdio_sync_filebuf<_CharT>* cout_buf_ptr = dynamic_cast<stdio_sync_filebuf<_CharT>*>(os->rdbuf());
			if (cout_buf_ptr)
			{
				FILE* f = cout_buf_ptr->file();
				if (isatty(f->_fileno))
					return true;
			}
			return false;
